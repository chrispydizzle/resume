...
function wpbandit_pre_get_posts( $query ) {
	...
	if ( $query->is_home() ) {
		...
		if ( wpb_option( 'featured-slider-enable' ) ) {
			// Get featured post ids
			$featured_post_ids = wpb_get_featured_post_ids();
			// Exclude posts
			if ( $featured_post_ids && ! wpb_option( 'featured-slider-include' ) ) {
				$query->set( 'post__not_in', $featured_post_ids );
			}
		}


		// Highlighted posts enabled
		if ( wpb_option( 'highlights-enable' ) ) {
			// Get highlighted post ids
			$highlights_post_ids = wpb_get_highlights_post_ids();
			// Exclude posts
			if ( $highlights_post_ids && ! wpb_option( 'highlights-include' ) ) {
				if ( $query->get( 'post__not_in' ) ) {
					$posts_not_in = array_merge( $query->get( 'post__not_in' ), $highlights_post_ids );
					$posts_not_in = array_unique( $posts_not_in );
					$query->set( 'post__not_in', $posts_not_in );
				} else {
					$query->set( 'post__not_in', $highlights_post_ids );
				}
			}
		}
	}
...
function wpbandit_advanced_css( $section, $valid ) {
	...
	if ( $body_bg_image_repeat ) {
		$styles .= 'background-repeat: ' . $body_bg_image_repeat . '; background-position: top center; ';
	}
	$styles .= '}' . "\n";
...
/**
 * Get highlights post ids
 **/
function wpb_get_highlights_post_ids() {
	// Set arguments
	$args = array(
		'category'    => Air::get_option( 'highlights-category' ),
		'numberposts' => 3
	);
